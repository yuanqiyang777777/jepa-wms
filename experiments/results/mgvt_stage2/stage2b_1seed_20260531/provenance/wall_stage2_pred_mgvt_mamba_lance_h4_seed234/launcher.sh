#!/bin/bash
# Run the MGVT-JEPA Stage-2b H=4 rollout-stability breadth gate on lab01.
#
# Default full scan: 3 backbones x 5 Phase-1 tasks x 1 seed.
# Smoke example:
#   QUICK_DEBUG=1 CONFIG_FILTER=wall_stage2_pred_adaln_depth1_lance_h4 \
#   RUN_ROOT="$JEPAWM_LOGS/mgvt_stage2b_smoke" \
#   CKPT_ROOT="$JEPAWM_CKPT/mgvt_stage2b_smoke" bash experiments/scripts/mgvt_stage2b_scan.sh
...
RUN_ROOT="${RUN_ROOT:-$JEPAWM_LOGS/mgvt_stage2b_1seed}"
CKPT_ROOT="${CKPT_ROOT:-$JEPAWM_CKPT/mgvt_stage2b_1seed}"
SKILL_DEVICE="${SKILL_DEVICE:-cuda:0}"
SKILL_MAX_HORIZON="${SKILL_MAX_HORIZON:-6}"
PUSHT_SEEDS="${PUSHT_SEEDS:-234}"
WALL_SEEDS="${WALL_SEEDS:-234}"
MAZE_SEEDS="${MAZE_SEEDS:-234}"
MW_R_SEEDS="${MW_R_SEEDS:-1}"
...
if [ "$REQUIRE_LANCE_STORES" = "1" ]; then
  ...
  for uri in \
    ...
    if [ ! -e "$uri" ]; then
      ...
      echo "Build the store first; Stage-2b never falls back to raw data." >&2
      exit 2
    fi
  done
fi
...
CONFIGS=(
  "configs/vjepa_wm/mgvt_stage2b/pusht_stage2_pred_adaln_depth1_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/pusht_stage2_pred_mgvt_convmixer_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/pusht_stage2_pred_mgvt_mamba_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/wall_stage2_pred_adaln_depth1_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/wall_stage2_pred_mgvt_convmixer_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/wall_stage2_pred_mgvt_mamba_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/maze_stage2_pred_adaln_depth1_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/maze_stage2_pred_mgvt_convmixer_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/maze_stage2_pred_mgvt_mamba_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/mw_r_stage2_pred_adaln_depth1_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/mw_r_stage2_pred_mgvt_convmixer_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/mw_r_stage2_pred_mgvt_mamba_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/mw_rw_stage2_pred_adaln_depth1_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/mw_rw_stage2_pred_mgvt_convmixer_lance_h4.yaml"
  "configs/vjepa_wm/mgvt_stage2b/mw_rw_stage2_pred_mgvt_mamba_lance_h4.yaml"
)

mkdir -p "$RUN_ROOT" "$CKPT_ROOT" "$RUN_ROOT/skill_scores" "$RUN_ROOT/summary"

{
  echo "MGVT Stage-2b scan starting $(date -Iseconds)"
...
raw_csv = summary_dir / "mgvt_stage2b_horizon_raw.csv"
...
summary_csv = summary_dir / "mgvt_stage2b_horizon_summary.csv"
...
(summary_dir / "mgvt_stage2b_horizon_summary.md").write_text("\n".join(md_lines) + "\n")
print("\n".join(md_lines))
PY

echo "MGVT Stage-2b scan finished $(date -Iseconds)" | tee "$RUN_ROOT/scan_end.txt"
