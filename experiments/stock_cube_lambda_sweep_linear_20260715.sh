...
RUN_ID=stock_cube_lambda_sweep_linear_20260715
ROOT="$BASE/$RUN_ID"
SOURCE_RUN_ROOT="$BASE/stock_cube_probe_20260714"
SOURCE_JEPA="$CODE/jepa-wms"
SOURCE_STABLE="$CODE/stable-worldmodel"
JEPA_WT="$CODE/wt_stock_cube_pusht_probe_jepawms_20260714"
SWM_WT_REUSE="$CODE/wt_probe_f2compat_stablewm_20260713"
SWM_WT_ALT="$CODE/wt_stock_cube_probe_stablewm_20260714"
JEPA_BASE_COMMIT=21389db737ddf1bcbc8dc695d27ca8f7a8b77cb0
STABLE_COMMIT=314201d0347baf61304c7558a98463c6a01080f0
EXPECTED_BRANCH=exp/20260714-stock-cube-pusht-probe
...
SOURCE_MANIFEST="$JEPA_WT/experiments/stock_cube_probe_manifest_20260714.json"
SOURCE_CACHE="$JEPA_WT/experiments/scripts/stock_cube_lambda_sweep_linear_cache_20260715.py"
SOURCE_LAUNCHER="$JEPA_WT/experiments/stock_cube_lambda_sweep_linear_20260715.sh"
...
EXPECTED_MANIFEST=1e769c4c9b7876374f5c059af351476b2a71a73be37156cabc26b328ac0a5561
EXPECTED_ENV_SOURCE=143e5e81f424ff7a4d7fdde287dee88b722c09082a6fc9e68721839788e94c19
EXPECTED_BASELINE_SHA256=433cc4df1f963979941d7f78d0ce41b0043f64ac6a9ab353a47ef17861247da9
# Review identity only.  The analyzer is local-only and is never copied to lab01.
EXPECTED_ANALYZER_SHA256=e305b419d0d44b383b2d38da767febebcb637546abfd73c45db3de0bba3225f0
...
CELLS_JSON='["l1_linear_l03","l2_linear_l04","l3_linear_l06","l4_linear_l07"]'
...
MANIFEST="$PROV/stock_cube_probe_manifest_20260714.json"
CACHE="$PROV/stock_cube_lambda_sweep_linear_cache_20260715.py"
LAUNCHER="$PROV/stock_cube_lambda_sweep_linear_20260715.sh"
ENV_SOURCE="$PROV/cube_env_at_314201d0.py"
...
checkpoint_path() {
  local seed=$1
  printf '%s/checkpoints/stock_lewm_cube_matched3_seed%s_20260615/weights_epoch_10.pt\n' \
    "$BASE" "$seed"
}
...
checkpoint_sha() {
  case "$1" in
    42) echo 800184d028b3c7af8f3cb69f889307bc7a2433e911cbbcba92f5ddc265b1eb07 ;;
    43) echo ef44632fdb57d5c1ea28f52865c38ca9797ad7dd93f9ee542dc78a081937f700 ;;
    44) echo 96e8e4401a330c08282efd5708f58240d500cf960648626ff0b084b1ef8148b3 ;;
...
config_sha() {
  case "$1" in
    42) echo d0ca995d841ad41f207fb0ddacd1d35187dfd0faf53c6baf20f7ac8d09afb8e4 ;;
    43) echo 3f52e56d9c809065c80a6082ba832b62462fc61f552f8d4d8b1e67dc360feb79 ;;
    44) echo 545b8e1c5e51b364395fa2a1e74c24022d200f6e7330f3a14600c562b2c445a1 ;;
    *) die "unsupported stock train seed: $1" ;;
  esac
}
...
require_checkpoints() {
  ...
  [[ -f "$DATASET" ]] || die "missing Cube dataset: $DATASET"
}
...
require_dataset_identity() {
  ...
  [[ "$actual" == "$expected" ]] \
    || die "Cube dataset hash drift: expected=$expected actual=$actual"
}
...
require_dataset_stat() {
  ...
  [[ "$current" == "$(<"$PROV/cube_dataset_stat.txt")" ]] \
    || die "Cube dataset stat drift"
}
...
require_jepa_branch() {
  ...
  while IFS=$'\t' read -r diff_status diff_path; do
    ...
    case "$diff_path" in
      experiments/stock_cube_probe_20260714.sh|experiments/scripts/stock_cube_probe_cache_20260714.py|experiments/stock_cube_probe_manifest_20260714.json|experiments/stock_pusht_probe_20260714.sh|experiments/scripts/stock_pusht_probe_cache_20260714.py|experiments/stock_pusht_probe_manifest_20260714.json|experiments/stock_cube_lambda_sweep_linear_20260715.sh|experiments/scripts/stock_cube_lambda_sweep_linear_cache_20260715.py|experiments/stock_cube_lambda_sweep_mlp_20260715.sh|experiments/scripts/stock_cube_lambda_sweep_mlp_cache_20260715.py)
        ;;
  ...
  [[ "$diff_count" -eq 10 ]] \
    || die "expected exactly ten added experiment files, found $diff_count"
}
...
require_stable_worktree() {
  ...
  env_file="$selected/stable_worldmodel/envs/ogbench/cube_env.py"
...
setup_run() {
  ...
  env_file="$selected_swm/stable_worldmodel/envs/ogbench/cube_env.py"
  require_hash "$env_file" "$EXPECTED_ENV_SOURCE"
...
run_cache_internal() {
  ...
  swm=$(stable_worktree)
  cd "$JEPA_WT"
  write_command "$COMMANDS/cache_fit.command.txt" "$PY" "$CACHE" import-caches \
    --source-root "$SOURCE_RUN_ROOT" --run-root "$ROOT"
  set +e
  PYTHONPATH="$swm${PYTHONPATH:+:$PYTHONPATH}" ADADIM_STABLEWM_REPO="$swm" WANDB_MODE=disabled \
    OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 OPENBLAS_NUM_THREADS=8 NUMEXPR_NUM_THREADS=8 \
    setsid "$PY" "$CACHE" import-caches \
      --source-root "$SOURCE_RUN_ROOT" --run-root "$ROOT" &
  WORKER_CHILD_PID=$!
...
start_cache() {
  ...
  atomic_text "$STATE/cache.attempted.json" \
    "{\"state\":\"CACHE_ATTEMPTED\",\"gpu\":0,\"launch_token\":\"$launch_token\",\"dataset_sha256\":\"$(<"$PROV/cube_dataset_sha256.txt")\",\"baseline_manifest_sha256\":\"$EXPECTED_BASELINE_SHA256\",\"at\":\"$(iso_now)\"}"
...
spec = importlib.util.spec_from_file_location("stock_cube_cache_reviewed", cache_path)
...
if (
    ...
    or summary.get("run_id") != module.SOURCE_RUN_ID
...
cell_gpu() {
  case "$1" in
    l1_linear_l03) echo 0 ;;
    l2_linear_l04) echo 1 ;;
    l3_linear_l06) echo 2 ;;
    l4_linear_l07) echo 3 ;;
    *) die "unknown cell: $1" ;;
  esac
}
...
expected = {
    ...
    "baseline_manifest_sha256": baseline_sha,
    "dataset_sha256": (root / "provenance" / "cube_dataset_sha256.txt").read_text(
        encoding="utf-8"
    ).strip(),
...
preflight_batch() {
  ...
  for cell in "${CELLS[@]}"; do
    gpu=$(cell_gpu "$cell")
    gpu_free "$gpu"
    [[ ! -e "$ROOT/$phase/cube_${cell}" \
        && ! -e "$ROOT/$phase/cube_${cell}.partial" ]] \
...
run_cell_internal() {
  ...
  cell_is_fixed "$cell" || die "cell is not one of frozen unconditional C1-C4: $cell"
...
spec = importlib.util.spec_from_file_location("stock_cube_cache_runtime", cache_path)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
cells = list(module.CELL_SPECS)
expected_dirs = {f"cube_{cell}" for cell in cells}
...
dataset_sha = (root / "provenance" / "cube_dataset_sha256.txt").read_text(
    encoding="utf-8"
).strip()
...
if len(pid_lines) != 4 or [line[0] for line in pid_lines] != cells:
    raise SystemExit(f"{mode} pid ledger does not contain ordered unconditional C1-C4")
...
for cell in cells:
    cell_root = phase_root / f"cube_{cell}"
    ...
    for row in episodes:
        ...
        if (
            row["task"] != "cube" or row["arm"] != "stock"
    ...
    if mode == "full":
        if module.sha256_json(runtime_arrays) != "9d3e5fec7279dbfb81176b01fb5a1a8062ed3a07b8b2932146f4bbfede7cfca9":
            raise SystemExit(f"{cell} ordered full runtime arrays differ from frozen baseline")
        if module.sha256_json(runtime_keys) != "5d776e6fc32b6a1552843021d3840e3cb8be6ef339263e8d58291738e924e0dc":
    ...
    for summary in summaries:
        ...
        expected_checkpoint = (
            ...
            f"stock_lewm_cube_matched3_seed{ts}_20260615/weights_epoch_10.pt"
        )
        if (
            summary.get("task") != "cube" or summary.get("arm") != "stock"
        ...
        if (
            ...
            or adapter.get("target_names") != ["block_position"]
            or int(adapter.get("target_dim", -1)) != 3
    ...
    for row in replans:
        ...
        if (
            row["task"] != "cube" or row["arm"] != "stock" or row["method"] != "fixed_full"
...
payload = {
    "schema_version": "stock_cube_remote_runtime_validation_v1",
    "run_id": module.RUN_ID,
    "state": state,
    "mode": mode,
    "cells": cells,
...
spec = importlib.util.spec_from_file_location("stock_cube_cache_attestation", cache_path)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
runtime = root / f"{phase}_runtime_validated.json"
payload = {
...
assert_no_live_workers() {
  ...
  for proc in /proc/[0-9]*; do
    ...
    if [[ "$command_line" == *"$CACHE"* && "$command_line" == *"$ROOT"* \
          && ( "$command_line" == *"import-caches"* || "$command_line" == *"run-cached"* ) ]]; then
...
usage() {
  cat <<'EOF'
usage: stock_cube_lambda_sweep_linear_20260715.sh COMMAND
