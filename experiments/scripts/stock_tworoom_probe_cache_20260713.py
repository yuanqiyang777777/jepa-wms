#!/usr/bin/env python3
"""Fit immutable stock-LeWM TwoRoom probe caches and run from them.

This is the only probe implementation for the preregistered 2026-07-13 stock
TwoRoom prospective-gate experiment.  Its result-bearing operations are:
...
PURE_GATE_THRESHOLD_PX = SUCCESS_RADIUS_PX / 5.0
HYBRID_GATE_THRESHOLD_PX = SUCCESS_RADIUS_PX / 3.0
...
TASK_MANIFEST_SHA256 = "ffffb1ae170ec45db59a7b155890ef868e145a30e112a580e99bfc14bf4987ad"
BASELINE_MANIFEST_SHA256 = "474065cefd579152dc670cbcc5896d7d97790b887e1ee93946c3d2bfd48c0640"
BASELINE_EVAL_ARRAYS_SHA256 = "c50386fa3d88071033327ca31ad1e6d84074b89117f36265bb2e8fb1b2d155cb"
BASELINE_PAIR_EVAL_ARRAYS_SHA256 = {
    (42, 42): "6f1567934ffb1d0f87debde5e1093104ba69686d5ae2044c7f5d71209ef59597",
    (42, 43): "0e0c6d3bd0752059a0957c57f32e176a789370df58cbb83042a464ea76d8effa",
    (42, 44): "09b0c69fe4aa931a998cf0ce254ff1000d05aa0f753744c5aecf0e01807b3728",
    (42, 45): "f5bc08f9bf80d42c9b3723d3fe2d3f2ef02683385c6876273f1c3aaf73f254b3",
    (42, 46): "a6a7940ac948e4c969fa950af3acf444c51ed7a4b6047f062342bb5e3f902afd",
    (43, 42): "d9be888e851473a93e909fafd9e28505edd3754b27848fd70c066db842559a8e",
    (43, 43): "5d211504428a25cb5729bb479d0ac18327344f1c09b3bb80d94d2d61786dd9a3",
    (43, 44): "77ba6a4a6857cfefbd11b426eff4afa0ace08c51c212cb09bf2ddf5c3c749f12",
    (43, 45): "2e2ecb4b8d78a48f066929fdba63f2fba8197512343386aaeec971cdfdc1109e",
    (43, 46): "1483bd85d3547cdaf6d96c26e6fed3b8c010b61cc1c58dc56fd4362303d263f1",
    (44, 42): "a66220438675330610a67572e62e34c758f2162f43d18f35df487fb18f875c53",
    (44, 43): "770d89236a3eb7a3798fc17c00b3376eaab1becaff0c39e8c03b65540fe4ecf1",
    (44, 44): "d4d448c150938faf1fa02e69ffabe5c03b9fcbc7e509585fac77b955c8fbe53c",
    (44, 45): "c4aa44e9805b969906d3e3318fac8e838b370cb1dd62030754e358298b292a44",
    (44, 46): "9cae3df26491ab4d1d7b9e57d932beeb8331f29115df1dbdb070b98b2d38fc7d",
}
JEPA_WMS_BASE_COMMIT = "3214dc52ff3290256ef77b985c9e1f1e339fde44"
STABLE_WORLDMODEL_COMMIT = "314201d0347baf61304c7558a98463c6a01080f0"
CHECKPOINT_SHA256 = {
    42: "96da8768f4e51707417b548f00efb19bceab39adff7da49b1a6e50b4d3491d56",
    43: "9b6cdf2fa1d8a6f89512201aa422142c7158220d83eebd7b532d4ae5ed560973",
    44: "43d783e3950231c17441e81b7bbe76c2d81ab1d1fb260be9af894519a484231e",
}
CONFIG_SHA256 = {
    42: "9e0362012040493d793d6297d658fd729d9bc1e15443dd33534f39e1d5505938",
    43: "a2165f9c8541b5d97fd6253d3ef6ee05828e32d944134115605cd97a1567a0f6",
    44: "76aea934e7e3e96288c18829b256da4589b76aafa36d4084fdee58abea04aaf4",
}
...
CELL_SPECS: dict[str, dict[str, Any]] = {
    "t1_linear_pure": {"family": "s0", "hybrid_lambda": 1.0, "mode": "pure"},
    "t2_linear_hybrid_l05": {"family": "s0", "hybrid_lambda": 0.5, "mode": "hybrid"},
    "t3_mlp_hybrid_l05": {"family": "s1", "hybrid_lambda": 0.5, "mode": "hybrid"},
    "t4_mlp_pure": {"family": "s1", "hybrid_lambda": 1.0, "mode": "pure"},
}

SCHEMA_VERSION = "stock_tworoom_probe_cache_v1"
HEX40 = re.compile(r"[0-9a-f]{40}")
HEX64 = re.compile(r"[0-9a-f]{64}")
STOCK_MODEL_RE = re.compile(r"stock_seed(?P<seed>42|43|44)")

DATASET_UNIT_CONTRACT = {
    "coordinate_frame": "TwoRoom environment pixels",
    "image_extent_px": [224, 224],
    "position_fields": ["pos_agent", "proprio", "pos_target"],
    "distance_field": "distance_to_target",
    "position_atol": 1.0e-5,
    "distance_atol_px": 1.0e-3,
}
...
def _dataset_unit_attestation(dataset: Path) -> dict[str, Any]:
    """Prove that the probe target and r=16 gate use the same pixel frame."""
    import h5py

    required = {"pos_agent", "proprio", "pos_target", "distance_to_target", "ep_idx"}
    ...
    with h5py.File(dataset, "r") as handle:
        ...
        if missing:
            raise ValueError(f"TwoRoom dataset lacks unit-contract fields: {missing}")
        for name in ("pos_agent", "proprio", "pos_target"):
            value = handle[name]
            if value.ndim != 2 or value.shape[1] != 2:
                raise ValueError(f"TwoRoom {name} must have shape (N,2), got {value.shape}")
            if n_rows is None:
                n_rows = int(value.shape[0])
            elif int(value.shape[0]) != n_rows:
                raise ValueError("TwoRoom coordinate fields have different row counts")
        distance = handle["distance_to_target"]
        if distance.ndim not in (1, 2) or (distance.ndim == 2 and distance.shape[1] != 1):
            raise ValueError(
                "TwoRoom distance_to_target must have shape (N,) or (N,1), "
                f"got {distance.shape}"
            )
        if int(distance.shape[0]) != n_rows or int(handle["ep_idx"].shape[0]) != n_rows:
            raise ValueError("TwoRoom unit-contract fields have different row counts")
        assert n_rows is not None
        for start in range(0, n_rows, chunk):
            stop = min(start + chunk, n_rows)
            agent = np.asarray(handle["pos_agent"][start:stop], dtype=np.float64)
            proprio = np.asarray(handle["proprio"][start:stop], dtype=np.float64)
            target = np.asarray(handle["pos_target"][start:stop], dtype=np.float64)
            dist = np.asarray(distance[start:stop], dtype=np.float64).reshape(-1)
            if not (
                np.isfinite(agent).all()
                and np.isfinite(proprio).all()
                and np.isfinite(target).all()
                and np.isfinite(dist).all()
            ):
                raise ValueError("TwoRoom physical coordinate fields contain NaN/Inf")
            minimum_position = min(
                minimum_position, float(agent.min()), float(target.min()), float(proprio.min())
            )
            maximum_position = max(
                maximum_position, float(agent.max()), float(target.max()), float(proprio.max())
            )
            maximum_position_delta = max(
                maximum_position_delta, float(np.max(np.abs(agent - proprio)))
            )
            recomputed = np.linalg.norm(agent - target, axis=1)
            maximum_distance_delta = max(
                maximum_distance_delta, float(np.max(np.abs(recomputed - dist)))
            )
    if minimum_position < -DATASET_UNIT_CONTRACT["position_atol"]:
        raise ValueError(f"TwoRoom pixel coordinate below zero: {minimum_position}")
    if maximum_position > 224.0 + DATASET_UNIT_CONTRACT["position_atol"]:
        raise ValueError(f"TwoRoom pixel coordinate above 224: {maximum_position}")
    if maximum_position_delta > DATASET_UNIT_CONTRACT["position_atol"]:
        raise ValueError(
            "TwoRoom pos_agent and proprio differ: "
            f"max_abs_delta={maximum_position_delta}"
        )
    if maximum_distance_delta > DATASET_UNIT_CONTRACT["distance_atol_px"]:
        raise ValueError(
            "TwoRoom distance_to_target is not Euclidean pixel distance: "
            f"max_abs_delta={maximum_distance_delta}"
        )
    payload = {
        **DATASET_UNIT_CONTRACT,
        ...
        "max_abs_pos_agent_minus_proprio_px": maximum_position_delta,
        "max_abs_distance_reconstruction_error_px": maximum_distance_delta,
        "status": "PASS",
    }
...
def _validate_dataset_unit_attestation(payload: Any) -> None:
    if not isinstance(payload, dict):
        raise ValueError("cache lacks the TwoRoom dataset-unit attestation")
    actual = dict(payload)
    recorded = actual.pop("attestation_sha256", None)
    if recorded != sha256_json(actual):
        raise ValueError("TwoRoom dataset-unit attestation hash drift")
    for name, expected in DATASET_UNIT_CONTRACT.items():
        if actual.get(name) != expected:
            raise ValueError(f"TwoRoom unit-contract field drift: {name}")
    if actual.get("status") != "PASS" or int(actual.get("n_rows_checked", 0)) <= 0:
        raise ValueError("TwoRoom dataset-unit attestation is not a positive PASS")
    if float(actual["max_abs_pos_agent_minus_proprio_px"]) > float(
        DATASET_UNIT_CONTRACT["position_atol"]
    ):
        raise ValueError("TwoRoom position-unit attestation exceeds tolerance")
    if float(actual["max_abs_distance_reconstruction_error_px"]) > float(
        DATASET_UNIT_CONTRACT["distance_atol_px"]
    ):
        raise ValueError("TwoRoom distance-unit attestation exceeds tolerance")
...
def _assert_probe_partition(
    ...
    with h5py.File(dataset, "r") as handle:
        ...
        if int(indices.min()) < 0 or int(indices.max()) >= n_rows:
            raise ValueError("probe sample index is outside the TwoRoom dataset")
...
def _assert_tworoom_target_contract(budget_meta: dict[str, Any]) -> None:
    for budget in BUDGETS:
        metadata = budget_meta[str(budget)]
        if (
            int(metadata.get("target_dim", -1)) != 2
            or list(metadata.get("target_names", [])) != ["agent_position"]
            or [list(block) for block in metadata.get("target_blocks", [])] != [[0, 2]]
        ):
            raise ValueError(
                "TwoRoom success probe must decode agent_position in the 2-D pixel frame: "
...
def _base_cache_metadata(
    *,
    ...
    return {
        ...
        "pure_gate_threshold_px": PURE_GATE_THRESHOLD_PX,
        "hybrid_gate_threshold_px": HYBRID_GATE_THRESHOLD_PX,
        "physical_coordinate_unit": "TwoRoom environment pixels",
...
def _validate_checkpoint(checkpoint: Path, train_seed: int) -> None:
    ...
    expected_name = f"stock_lewm_tworoom_matched3_seed{train_seed}_20260615"
...
def fit_cache(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for train_seed in TRAIN_SEEDS:
        ...
        for eval_seed in EVAL_SEEDS:
            ...
            for family in ("s0", "s1"):
                ...
                _assert_tworoom_target_contract(budget_meta)
    ...
    summary = {
        ...
        "prospective_gate_input": {
            "aggregation": "per-family strict max over all 15 A192 caches and every target coordinate",
            "success_radius_px": SUCCESS_RADIUS_PX,
            "pure_threshold_px": PURE_GATE_THRESHOLD_PX,
            "hybrid_threshold_px": HYBRID_GATE_THRESHOLD_PX,
...
def recompute_prospective_predictions(
    ...
    """Verify all caches and recompute the preregistered prospective verdicts."""
    ...
    for family in ("s0", "s1"):
        ...
        for train_seed in TRAIN_SEEDS:
            for eval_seed in EVAL_SEEDS:
                ...
                if (
                    int(budget_meta["target_dim"]) != 2
                    or budget_meta["target_names"] != ["agent_position"]
                    or len(coordinates) != 2
                    or not all(math.isfinite(x) for x in coordinates)
                ):
                    raise ValueError(
                        "TwoRoom A192 gate coordinates invalid for "
                        f"family={family} train={train_seed} eval={eval_seed}"
                    )
                values.extend(coordinates)
        if len(values) != 30:
            raise ValueError(
                f"family {family} expected 30 A192 coordinates, found {len(values)}"
            )
        worst = max(values)
        family_results[family] = {
            "probe_family": FAMILY_SPECS[family]["probe_family"],
            "aggregation": "strict max over all 15 A192 caches and every target coordinate",
            "target_name": "agent_position",
            "target_dim": 2,
            "n_caches": 15,
            "n_coordinates": 30,
            "worst_rmse_phys_px": worst,
            "pure": {
                "threshold_px": PURE_GATE_THRESHOLD_PX,
                "gate_verdict": "PASS" if worst <= PURE_GATE_THRESHOLD_PX else "FAIL",
            },
            "hybrid": {
                "threshold_px": HYBRID_GATE_THRESHOLD_PX,
                "gate_verdict": "PASS" if worst <= HYBRID_GATE_THRESHOLD_PX else "FAIL",
            },
        }
    ...
    for cell, spec in CELL_SPECS.items():
        ...
        verdict = family_results[family][mode]["gate_verdict"]
        cell_predictions[cell] = {
            "family": family,
            "mode": mode,
            "gate_verdict": verdict,
            "prediction": (
                "no_significant_regression_vs_true_stock"
                if verdict == "PASS"
                else "significant_regression_vs_true_stock"
            ),
            "launch_policy": "RUN_REGARDLESS_OF_GATE_VERDICT",
        }
    return {
        "schema_version": "stock_tworoom_prospective_gate_v1",
        "run_id": RUN_ID,
        ...
        "planning_budget": 192,
        "success_radius": {
            "value_px": SUCCESS_RADIUS_PX,
            "pure_threshold_px": PURE_GATE_THRESHOLD_PX,
            "hybrid_threshold_px": HYBRID_GATE_THRESHOLD_PX,
            "source_commit": STABLE_WORLDMODEL_COMMIT,
            "source_file": "stable_worldmodel/envs/two_room/env.py",
            "condition": "terminated = dist < 16.0",
        },
        "prediction_scoring": {
            "pass": "predict no significant regression vs true-stock baseline",
            "fail": "predict significant regression vs true-stock baseline",
            "realized_test": "two-sided exact McNemar raw alpha=0.05; scoring only",
        },
...
def publish_prospective_predictions(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if list((run_root / "smoke").glob("tworoom_*")) or list(
        (run_root / "full").glob("tworoom_*")
    ):
...
def _validate_loaded_metadata(
    ...
    *,
    ...
    _assert_tworoom_target_contract(metadata.get("budget_metadata", {}))
...
def _validate_planning_ordering_state(
    ...
    *,
    ...
    recorded_dataset_sha = _require_sha256(
        (provenance / "tworoom_dataset_sha256.txt").read_text(encoding="utf-8").strip(),
    ...
    def tracked_import_budget_runner(path: Path) -> Any:
        ...
        def tracked_prepare_common(
            ...
            if Path(run_spec["dataset"]).resolve() != expected_dataset:
                raise ValueError("prepare_common TwoRoom dataset path drift")
            _assert_plain_stock_model(prepared["model"])
            if args.mode == "full":
