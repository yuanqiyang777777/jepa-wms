#!/usr/bin/env python3
"""Fit immutable stock-LeWM Cube probe caches and run from them.

This is the only probe implementation for the preregistered 2026-07-14 stock
Cube prospective-gate experiment.  Its result-bearing operations are:
...
CELL_SPECS: dict[str, dict[str, Any]] = {
    "c1_linear_pure": {"family": "s0", "hybrid_lambda": 1.0, "mode": "pure"},
    "c2_linear_hybrid_l05": {"family": "s0", "hybrid_lambda": 0.5, "mode": "hybrid"},
    "c3_mlp_hybrid_l05": {"family": "s1", "hybrid_lambda": 0.5, "mode": "hybrid"},
    "c4_mlp_pure": {"family": "s1", "hybrid_lambda": 1.0, "mode": "pure"},
}
...
def fit_cache(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for train_seed in TRAIN_SEEDS:
        ...
        for eval_seed in EVAL_SEEDS:
            ...
            for family in ("s0", "s1"):
                ...
                _assert_cube_target_contract(budget_meta)
    ...
    summary = {
        ...
        "prospective_gate_input": {
            ...
            "success_radius_m": SUCCESS_RADIUS_M,
            "pure_threshold_m": PURE_GATE_THRESHOLD_M,
            "hybrid_threshold_m": HYBRID_GATE_THRESHOLD_M,
...
def recompute_prospective_predictions(
    ...
    for family in ("s0", "s1"):
        ...
        for train_seed in TRAIN_SEEDS:
            for eval_seed in EVAL_SEEDS:
                ...
                if (
                    int(budget_meta["target_dim"]) != 3
                    or budget_meta["target_names"] != ["block_position"]
                    or len(coordinates) != 3
                    or not all(math.isfinite(x) for x in coordinates)
                ):
                    raise ValueError(
                        "Cube A192 gate coordinates invalid for "
                        f"family={family} train={train_seed} eval={eval_seed}"
                    )
                values.extend(coordinates)
        if len(values) != 45:
            raise ValueError(
                f"family {family} expected 45 A192 coordinates, found {len(values)}"
            )
        worst = max(values)
        family_results[family] = {
            ...
            "target_name": "block_position",
            "target_dim": 3,
            "n_caches": 15,
            "n_coordinates": 45,
            "worst_rmse_phys_m": worst,
            "pure": {
                "threshold_m": PURE_GATE_THRESHOLD_M,
                "gate_verdict": "PASS" if worst <= PURE_GATE_THRESHOLD_M else "FAIL",
            },
            "hybrid": {
                "threshold_m": HYBRID_GATE_THRESHOLD_M,
                "gate_verdict": "PASS" if worst <= HYBRID_GATE_THRESHOLD_M else "FAIL",
            },
        }
    ...
    return {
        "schema_version": "stock_cube_prospective_gate_v1",
        "run_id": RUN_ID,
        ...
        "planning_budget": 192,
        "success_radius": {
            "value_m": SUCCESS_RADIUS_M,
            "pure_threshold_m": PURE_GATE_THRESHOLD_M,
            "hybrid_threshold_m": HYBRID_GATE_THRESHOLD_M,
            "source_commit": STABLE_WORLDMODEL_COMMIT,
            "source_file": "stable_worldmodel/envs/ogbench/cube_env.py",
            "condition": "success = ||block_pos - target_pos|| <= 0.04 m (_compute_successes)",
        },
        "prediction_scoring": {
