...
SWM_WT_ALT="$CODE/wt_stock_pusht_probe_stablewm_20260714"
...
SOURCE_MANIFEST="$JEPA_WT/experiments/stock_pusht_probe_manifest_20260714.json"
SOURCE_CACHE="$JEPA_WT/experiments/scripts/stock_pusht_probe_cache_20260714.py"
SOURCE_LAUNCHER="$JEPA_WT/experiments/stock_pusht_probe_20260714.sh"
...
EXPECTED_MANIFEST=7673dc4389b5cb0e9b4eb2082b3fbb1152c8307b790380eefdeee3c451c37ce3
...
CELLS=(
  p1_linear_pure
  p2_linear_hybrid_l05
  p3_mlp_hybrid_l05
  p4_mlp_pure
)
CELLS_JSON='["p1_linear_pure","p2_linear_hybrid_l05","p3_mlp_hybrid_l05","p4_mlp_pure"]'
...
MANIFEST="$PROV/stock_pusht_probe_manifest_20260714.json"
CACHE="$PROV/stock_pusht_probe_cache_20260714.py"
LAUNCHER="$PROV/stock_pusht_probe_20260714.sh"
ENV_SOURCE="$PROV/pusht_env_at_314201d0.py"
...
checkpoint_path() {
  local seed=$1
  printf '%s/checkpoints/stock_lewm_pusht_matched3_seed%s_20260615/weights_epoch_10.pt\n' \
    "$BASE" "$seed"
}
...
checkpoint_sha() {
  case "$1" in
    42) echo e94142169195edfb2fc7691a424d36b63d036722a5402aefdfa6ce1bd361b1d4 ;;
    43) echo 28c3a43e8604856cf92c50b18163b03ba4c56dc4f2be1e7ccf17b01ad40267fd ;;
    44) echo 90d78f11ec51271e81ba140a55303560c6dc31fe3f919b3b47f26ec15c34a98d ;;
...
config_sha() {
  case "$1" in
    42) echo 54fa420fb93bb253065408a976513c9b5ae4ef45ab8c60eeb12d6b324021b566 ;;
    43) echo c385ab390e84d5f28e75e47b198047912b7b10244a5e9e84300f41d9a63ff2a1 ;;
    44) echo 1b1bee67c98cc38950db6d53918f8ed71a809ae9b3f0af86ff08aa9a637262fb ;;
    *) die "unsupported stock train seed: $1" ;;
  esac
}
...
require_checkpoints() {
  ...
  [[ -f "$DATASET" ]] || die "missing PushT dataset: $DATASET"
}
...
require_dataset_identity() {
  [[ -f "$PROV/pusht_dataset_sha256.txt" ]] || die "dataset hash was not captured"
  local expected actual
  expected=$(<"$PROV/pusht_dataset_sha256.txt")
  actual=$(sha_of "$DATASET")
  [[ "$actual" == "$expected" ]] \
    || die "PushT dataset hash drift: expected=$expected actual=$actual"
}


require_dataset_stat() {
  [[ -f "$PROV/pusht_dataset_stat.txt" ]] || die "dataset stat was not captured"
  local current
  current=$(stat -c '%y %s %n' "$DATASET")
  [[ "$current" == "$(<"$PROV/pusht_dataset_stat.txt")" ]] \
    || die "PushT dataset stat drift"
}
...
require_jepa_branch() {
  ...
  while IFS=$'\t' read -r diff_status diff_path; do
    ...
    case "$diff_path" in
      experiments/stock_cube_probe_20260714.sh|experiments/scripts/stock_cube_probe_cache_20260714.py|experiments/stock_cube_probe_manifest_20260714.json|experiments/stock_pusht_probe_20260714.sh|experiments/scripts/stock_pusht_probe_cache_20260714.py|experiments/stock_pusht_probe_manifest_20260714.json)
        ;;
  ...
  [[ "$diff_count" -eq 6 ]] \
    || die "expected exactly six added experiment files, found $diff_count"
}
...
require_stable_worktree() {
  ...
  env_file="$selected/stable_worldmodel/envs/pusht/env.py"
...
setup_run() {
  ...
  env_file="$selected_swm/stable_worldmodel/envs/pusht/env.py"
  require_hash "$env_file" "$EXPECTED_ENV_SOURCE"
  ...
  stat -c '%y %s %n' "$DATASET" > "$PROV/pusht_dataset_stat.txt"
  sha_of "$DATASET" > "$PROV/pusht_dataset_sha256.txt"
...
start_cache() {
  ...
  atomic_text "$STATE/cache.attempted.json" \
    "{\"state\":\"CACHE_ATTEMPTED\",\"gpu\":0,\"launch_token\":\"$launch_token\",\"dataset_sha256\":\"$(<"$PROV/pusht_dataset_sha256.txt")\",\"baseline_manifest_sha256\":\"$EXPECTED_BASELINE_SHA256\",\"at\":\"$(iso_now)\"}"
...
spec = importlib.util.spec_from_file_location("stock_pusht_cache_reviewed", cache_path)
...
cell_gpu() {
  case "$1" in
    p1_linear_pure) echo 0 ;;
    p2_linear_hybrid_l05) echo 1 ;;
    p3_mlp_hybrid_l05) echo 2 ;;
    p4_mlp_pure) echo 3 ;;
    *) die "unknown cell: $1" ;;
  esac
}
...
expected = {
    ...
    "baseline_manifest_sha256": baseline_sha,
    "dataset_sha256": (root / "provenance" / "pusht_dataset_sha256.txt").read_text(
        encoding="utf-8"
    ).strip(),
...
preflight_batch() {
  ...
  for cell in "${CELLS[@]}"; do
    gpu=$(cell_gpu "$cell")
    gpu_free "$gpu"
    [[ ! -e "$ROOT/$phase/pusht_${cell}" \
        && ! -e "$ROOT/$phase/pusht_${cell}.partial" ]] \
...
run_cell_internal() {
  ...
  cell_is_fixed "$cell" || die "cell is not one of frozen unconditional P1-P4: $cell"
  [[ "${CUDA_VISIBLE_DEVICES:-}" == "$gpu" ]] \
    || die "$cell must use physical GPU $gpu"
  out_partial="$ROOT/$phase/pusht_${cell}.partial"
  out_final="$ROOT/$phase/pusht_${cell}"
...
start_batch() {
  ...
  dataset_sha=$(<"$PROV/pusht_dataset_sha256.txt")
...
require_phase_done() {
  ...
  for cell in "${CELLS[@]}"; do
    ...
    for required in run_metadata.json cell_summaries.json \
      probe_physical_cem_episode_records.csv probe_physical_cem_replan_records.csv \
      probe_physical_cem_summary.csv probe_physical_cem_summary.md; do
      [[ -f "$ROOT/$phase/pusht_${cell}/$required" ]] \
...
spec = importlib.util.spec_from_file_location("stock_pusht_cache_runtime", cache_path)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
cells = list(module.CELL_SPECS)
expected_dirs = {f"pusht_{cell}" for cell in cells}
...
dataset_sha = (root / "provenance" / "pusht_dataset_sha256.txt").read_text(
    encoding="utf-8"
).strip()
...
if len(pid_lines) != 4 or [line[0] for line in pid_lines] != cells:
    raise SystemExit(f"{mode} pid ledger does not contain ordered unconditional P1-P4")
...
for cell in cells:
    cell_root = phase_root / f"pusht_{cell}"
    ...
    for row in episodes:
        ...
        if (
            row["task"] != "pusht" or row["arm"] != "stock"
    ...
    if mode == "full":
        if module.sha256_json(runtime_arrays) != "53346d93b5c5b50e2dd922f5bca02cdbc177479b6bed9d30d2f018fd360c0b34":
            raise SystemExit(f"{cell} ordered full runtime arrays differ from frozen baseline")
        if module.sha256_json(runtime_keys) != "258401d6b8d76ee7cdeaa9bf42d87c1c9cee36e5cdb768c74dc5bb5be77f6613":
    ...
    for summary in summaries:
        ...
        expected_checkpoint = (
            ...
            f"stock_lewm_pusht_matched3_seed{ts}_20260615/weights_epoch_10.pt"
        )
        if (
            summary.get("task") != "pusht" or summary.get("arm") != "stock"
        ...
        if (
            ...
            or adapter.get("target_names") != ["agent_location", "block_location", "block_angle_sincos"]
            or int(adapter.get("target_dim", -1)) != 6
    ...
    for row in replans:
        ...
        if (
            row["task"] != "pusht" or row["arm"] != "stock" or row["method"] != "fixed_full"
...
payload = {
    "schema_version": "stock_pusht_remote_runtime_validation_v1",
    "run_id": module.RUN_ID,
    "state": state,
    "mode": mode,
    "cells": cells,
...
spec = importlib.util.spec_from_file_location("stock_pusht_cache_attestation", cache_path)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
runtime = root / f"{phase}_runtime_validated.json"
payload = {
...
usage() {
  cat <<'EOF'
usage: stock_pusht_probe_20260714.sh COMMAND
